...
void RunAndMeasure(const char* title, TFunc func) {
  ...
  std::cout << title << ":\t " << *times.begin() << "ms (max was "
            << *times.rbegin() << ") " << results[0] << '\n';
}
...
#pragma omp parallel for
    for (int i = 0; i < static_cast<int>(vec.size());
         ++i)  //  'i': index variable in OpenMP 'for' statement must have
  ...
  //	for (int i = 0; i < static_cast<int>(vec.size()); ++i) //  'i': index
  //variable in OpenMP 'for' statement must have signed integral type 		out[i] =
  //vec[i] * 2.0;
...
void TestTrig(const size_t vecSize) {
  ...
  RunAndMeasure("std::transform   ", [&vec, &out] {
    std::transform(vec.begin(), vec.end(), out.begin(), [](double v) {
      // we need to watch for the negative values! as sqrt will generate a
      // domain error....
  ...
  RunAndMeasure("std::transform seq", [&vec, &out] {
    std::transform(
        std::execution::seq, vec.begin(), vec.end(), out.begin(),
        [](double v) { return std::sqrt(std::sin(v) * std::cos(v)); });
    return out[0];
  });

  RunAndMeasure("std::transform par", [&vec, &out] {
    std::transform(
        std::execution::par, vec.begin(), vec.end(), out.begin(),
        [](double v) { return std::sqrt(std::sin(v) * std::cos(v)); });
    return out[0];
  });
...
#pragma omp parallel for
  ...
  //	for (int i = 0; i < static_cast<int>(vec.size()); ++i) //  'i': index
  //variable in OpenMP 'for' statement must have signed integral type 		out[i] =
  //std::sqrt(std::sin(vec[i])*std::cos(vec[i]));
...
void TestFresnel(const size_t vecSize) {
  ...
  std::generate(vec.begin(), vec.end(), []() {
    return glm::vec4(GenRandomFloat(-1.0f, 1.0f), GenRandomFloat(-1.0f, 1.0f),
  ...
  RunAndMeasure("std::transform   ", [&vec, &vecNormals, &vecFresnelTerms] {
    std::transform(vec.begin(), vec.end(), vecNormals.begin(),
  ...
  RunAndMeasure("std::transform seq", [&vec, &vecNormals, &vecFresnelTerms] {
    std::transform(std::execution::seq, vec.begin(), vec.end(),
                   vecNormals.begin(), vecFresnelTerms.begin(),
                   [](const glm::vec4& v, const glm::vec4& n) {
                     return fresnel(v, n, 1.0f);
                   });
    return vecFresnelTerms[0];
...
#pragma omp parallel for
  ...
  //	for (int i = 0; i < static_cast<int>(vec.size()); ++i) //  'i': index
  //variable in OpenMP 'for' statement must have signed integral type
  //		vecFresnelTerms[i] = fresnel(vec[i], vecNormals[i], 1.0f);
...
int main(int argc, char* argv[]) {
  ...
  if (step == 0 || step == 1) TestDoubleValue(vecSize);

  if (step == 0 || step == 2) TestTrig(vecSize);

  if (step == 0 || step == 3) TestFresnel(vecSize);
